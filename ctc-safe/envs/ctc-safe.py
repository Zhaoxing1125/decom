...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        self.traps = []
        #diameters for unsafe regions
        self.unsafe_d = [0.3, 0.4, 0.5]
        self.unsafe_times = {}
        for i, _ in enumerate(world.agents):
            self.unsafe_times[i] = [0, 0, 0]
            ...
            if l.alive:
                ...
                if np.random.uniform() <= l.respawn_prob:
                    ...
                    l.state.p_pos = np.random.uniform(low=-bound, high=bound,
                                                      size=world.dim_p)
                    while np.linalg.norm(l.state.p_pos - self.traps[0]) < self.unsafe_d[0] or \
                          np.linalg.norm(l.state.p_pos - self.traps[1]) < self.unsafe_d[1] or \
                          np.linalg.norm(l.state.p_pos - self.traps[2]) < self.unsafe_d[2]:
                        l.state.p_pos = np.random.uniform(low=-bound, high=bound,
                                                        size=world.dim_p)
    ...
    def reset_world(self, world):
        self.traps = []
        self.traps.append(np.random.uniform(low=-1+self.unsafe_d[0], high=1-self.unsafe_d[0], size=world.dim_p))
        self.traps.append(np.random.uniform(low=-1+self.unsafe_d[1], high=1-self.unsafe_d[1], size=world.dim_p))
        self.traps.append(np.random.uniform(low=-1+self.unsafe_d[2], high=1-self.unsafe_d[2], size=world.dim_p))
        ...
        for i, agent in enumerate(world.agents):
            agent.state.p_pos = np.random.uniform(low=-1, high=1,
                                                  size=world.dim_p)
            while np.linalg.norm(agent.state.p_pos - self.traps[0]) < self.unsafe_d[0] or \
                  np.linalg.norm(agent.state.p_pos - self.traps[1]) < self.unsafe_d[1] or \
                  np.linalg.norm(agent.state.p_pos - self.traps[2]) < self.unsafe_d[2]:
                agent.state.p_pos = np.random.uniform(low=-1, high=1,
                                                  size=world.dim_p)
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            landmark.state.p_pos = np.random.uniform(low=-bound, high=bound,
                                                     size=world.dim_p)
            while np.linalg.norm(landmark.state.p_pos - self.traps[0]) < self.unsafe_d[0] or \
                  np.linalg.norm(landmark.state.p_pos - self.traps[1]) < self.unsafe_d[1] or \
                  np.linalg.norm(landmark.state.p_pos - self.traps[2]) < self.unsafe_d[2]:
                landmark.state.p_pos = np.random.uniform(low=-bound, high=bound,
                                                  size=world.dim_p)
    ...
    def cost(self, agent, world):
        self.unsafe_times[agent.i] = [0, 0, 0]
        if np.linalg.norm(agent.state.p_pos - self.traps[0]) < self.unsafe_d[0]:
            self.unsafe_times[agent.i][0] = 1
        if np.linalg.norm(agent.state.p_pos - self.traps[1]) < self.unsafe_d[1]:
            self.unsafe_times[agent.i][1] = 1  
        if np.linalg.norm(agent.state.p_pos - self.traps[2]) < self.unsafe_d[2]:
            self.unsafe_times[agent.i][2] = 1        
        return self.unsafe_times[agent.i]
    ...
    def observation(self, agent, world):
        ...
        #unsafe region
        obs.append(agent.state.p_pos - self.traps[0])
        obs.append(np.array([np.linalg.norm(agent.state.p_pos - self.traps[0])]))
        obs.append(agent.state.p_pos - self.traps[1])
        obs.append(np.array([np.linalg.norm(agent.state.p_pos - self.traps[1])]))
        obs.append(agent.state.p_pos - self.traps[2])
        obs.append(np.array([np.linalg.norm(agent.state.p_pos - self.traps[2])]))        
