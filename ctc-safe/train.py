    ...
    for ep_i in range(0, config.n_episodes, config.n_rollout_threads):
        ...
        if len(replay_buffer) >= config.batch_size:
            ...
            for u_i in range(config.num_updates):
                ...
                model.update_cost_critic(sample, logger=logger)
                
                model.update_base_policies(sample, ep_i, logger=logger)
                
                model.update_pert_policy(sample, logger=logger)
                
                
                model.update_all_targets(ep_i)
        ...
        for a_i, a_ep_rew in enumerate(ep_rews):
            ...
            logger.add_scalar('agent%i/mean_episode_costs1' % a_i,
                              ep_costs[a_i][0] * config.episode_length, ep_i)
            logger.add_scalar('agent%i/mean_episode_costs2' % a_i,
                              ep_costs[a_i][1] * config.episode_length, ep_i)
            logger.add_scalar('agent%i/mean_episode_costs3' % a_i,
                              ep_costs[a_i][2] * config.episode_length, ep_i)
