...
from utils.policies import ContPolicy, ContPolicy_cost

class Agent(object):
    """
    General class for agents (policy, target policy)
    """
    def __init__(self, num_in_pol, num_out_pol, hidden_dim=64,
        ...
        self.pert_policy = ContPolicy_cost(num_in_pol, num_out_pol*4, num_out_pol,
                                     hidden_dim=hidden_dim,
                                     onehot_dim=onehot_dim)
        self.pert_target_policy = ContPolicy_cost(num_in_pol, num_out_pol*4,
                                            num_out_pol,
                                            hidden_dim=hidden_dim,
                                            onehot_dim=onehot_dim)

        hard_update(self.pert_target_policy, self.pert_policy)
        self.pert_policy_optimizer = Adam(self.pert_policy.parameters(), lr=lr*3, weight_decay=1e-5)
    ...
    def step2(self, obs, ac, explore=True, trgt=False):
        """
        Take a step forward in environment for a minibatch of observations
        Inputs:
            obs (PyTorch Variable): Observations for this agent
        ...
        """
        if not trgt:
            return self.pert_policy(obs, ac, sample=explore)
        else:
            return self.pert_target_policy(obs, ac, sample=explore)
