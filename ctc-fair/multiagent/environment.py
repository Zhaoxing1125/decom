...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None, cost_callback=None,
        ...
        # self.shared_reward = True # for rover tower
        self.time = 0
    ...
    def _step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            ...
            reward_n.append(rew)
            cost_n.append(self._get_cost(agent, rew, action_n[i]))
        ...
        #fairness 
        fairness = [np.std([x[0] for x in cost_n], axis=0)] * self.n
        #stability
        stability = [np.std(np.mean([x[1] for x in cost_n], axis=0))] * self.n
        
        if self.post_step_callback is not None:
            self.post_step_callback(self.world)
        return obs_n, reward_n, [[x, y] for x, y in zip(fairness, stability)], done_n, info_n
    ...
    def _get_cost(self, agent, rew, act):
        if self.cost_callback is None:
            return 0.0
        return self.cost_callback(agent, rew, act, self.world)
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                ...
                if self.force_discrete_action:
                    ...
                    ind = np.argmin([np.linalg.norm(action[0]-x) for x in b])
                    action[0][:] = b[ind]
