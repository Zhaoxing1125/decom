...
class Scenario(BaseScenario):
    ...
    def cost(self, agent, rew, act, world):
        cost_agent = [0, 0]
        #road distance
        self.fairness[agent.i].append(np.linalg.norm(act))
        cost_agent[0] = np.linalg.norm(act)
        #reward variance
        self.robustness[agent.i].append(rew)
        cost_agent[1] = self.robustness[agent.i]
        return cost_agent
